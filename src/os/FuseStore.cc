 ...
 * $cid/bitwise_hash_end = lowest hash value
...
static int os_mkdir(const char *path, mode_t mode)
{
  ...
  case FN_COLLECTION:
    if (cid.is_pg()) {
      // use the mode for the bit count.  because the usual umask of
      // 0002 will clobber the low world bits, shift them off.  e.g.,
      // mkdir --mode=037 mnt/0.7_head will create 0.7 with bits = 3.
      mode &= 0777;
      mode >>= 3;
      if (mode >= 32)
	return -EINVAL;
